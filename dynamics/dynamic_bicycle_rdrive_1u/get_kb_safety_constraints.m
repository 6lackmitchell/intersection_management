function [A,b,params] = get_kb_safety_constraints(t,x,settings)
...
row_idx  = nCons*Na+1:nCons*Na+2*Ng;
row_idxh = nCons*Na+1:nCons*Na+Ng;
...
%         LgH   = Lgh;
        ...
        if Ng > 0
            ...
            Aw(dd,Nu*Na+Ns+dd)   = -(LfH+alpha_H);
            Aw(dd+1,Nu*Na+Ns+dd) = +(LfH+alpha_H);
            bw(dd)               = 0; 
            bw(dd+1)             = +(LfH+alpha_H); 
        else
            Aw(dd,  1:Nu*Na)     = -LgH;
